...
CREATE TABLE IF NOT EXISTS public.groups (
  ...
  category TEXT CHECK (category IN ('weight_loss', 'muscle_gain', 'healthy_eating', 'running', 'fitness', 'general')),
...
CREATE TABLE IF NOT EXISTS public.challenges (
  ...
  challenge_type TEXT NOT NULL CHECK (challenge_type IN (
    'calories_burned',    -- Đốt cháy calo
    'calories_intake',    -- Nạp calo (cho người tăng cân)
    'steps',              -- Số bước chân
    'water_intake',       -- Uống nước (ml)
    'sleep_hours',        -- Giờ ngủ
    'workouts_completed', -- Số buổi tập
    'weight_loss',        -- Giảm cân (kg)
    'weight_gain',        -- Tăng cân (kg)
...
CREATE TABLE IF NOT EXISTS public.challenge_participants (
  ...
  daily_progress JSONB DEFAULT '[]'::jsonb, -- Array of {date, value}
...
CREATE TABLE IF NOT EXISTS public.posts (
  ...
  post_type TEXT DEFAULT 'general' CHECK (post_type IN (
    'general',           -- Bài đăng thông thường
    'meal',              -- Chia sẻ bữa ăn
    'workout',           -- Chia sẻ buổi tập
    'achievement',       -- Thành tựu
    'challenge_progress',-- Tiến độ thử thách
    'milestone',         -- Cột mốc (VD: giảm 5kg)
    'question'           -- Câu hỏi
  )),
...
CREATE TABLE IF NOT EXISTS public.notifications (
  ...
  type TEXT NOT NULL CHECK (type IN (
    'like',              -- Ai đó thích bài viết/bình luận
    'comment',           -- Ai đó bình luận
    'follow',            -- Ai đó theo dõi
    'mention',           -- Được nhắc đến
    'challenge_invite',  -- Được mời tham gia thử thách
    'challenge_start',   -- Thử thách bắt đầu
    'challenge_end',     -- Thử thách kết thúc
    'challenge_rank',    -- Thay đổi thứ hạng
    'group_invite',      -- Được mời vào nhóm
    'group_join',        -- Ai đó tham gia nhóm
...
CREATE TABLE IF NOT EXISTS public.reports (
  ...
  action_taken TEXT CHECK (action_taken IN ('none', 'warning', 'content_removed', 'user_banned', 'escalated')),
...
CREATE INDEX IF NOT EXISTS idx_bans_active ON public.bans(user_id, is_active) WHERE is_active = true;
...
CREATE TABLE IF NOT EXISTS public.achievements (
  ...
  description TEXT,
  icon TEXT NOT NULL,
  category TEXT CHECK (category IN ('challenge', 'streak', 'milestone', 'social', 'special')),
...
BEGIN
  ...
  VALUES (
    ...
    COALESCE(NEW.raw_user_meta_data->>'display_name', NEW.raw_user_meta_data->>'full_name', 'Người dùng mới'),
...
CREATE OR REPLACE FUNCTION public.update_challenge_ranks(p_challenge_id UUID)
RETURNS VOID AS $$
BEGIN
  WITH ranked AS (
    SELECT 
      id,
      ROW_NUMBER() OVER (ORDER BY current_value DESC, updated_at ASC) as new_rank
...
CREATE POLICY "Public challenges are viewable"
  ...
  USING (visibility = 'public' OR visibility = 'group');
...
CREATE POLICY "Users can view their own health records"
  ON public.user_health_records FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own health records"
  ON public.user_health_records FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own health records"
  ON public.user_health_records FOR UPDATE
  USING (auth.uid() = user_id);
...
CREATE POLICY "Admins have full access to bans"
...
INSERT INTO public.achievements (name, description, icon, category, requirement_type, requirement_value, points) VALUES
  ('Người mới', 'Tạo tài khoản thành công', '🎉', 'milestone', 'signup', 1, 10),
  ('Bước đầu', 'Hoàn thành thử thách đầu tiên', '🏆', 'challenge', 'challenges_completed', 1, 50),
  ('Chiến binh', 'Hoàn thành 10 thử thách', '⚔️', 'challenge', 'challenges_completed', 10, 200),
  ('Streak 7 ngày', 'Ghi nhận liên tục 7 ngày', '🔥', 'streak', 'streak_days', 7, 100),
  ('Streak 30 ngày', 'Ghi nhận liên tục 30 ngày', '💪', 'streak', 'streak_days', 30, 500),
  ('Kết nối', 'Có 10 người theo dõi', '👥', 'social', 'followers', 10, 50),
  ('Ảnh hưởng', 'Có 100 người theo dõi', '🌟', 'social', 'followers', 100, 200),
  ('Chia sẻ', 'Đăng 10 bài viết', '📝', 'social', 'posts', 10, 50),
  ('Giảm 5kg', 'Giảm được 5kg cân nặng', '⚖️', 'milestone', 'weight_lost', 5, 300),
  ('Giảm 10kg', 'Giảm được 10kg cân nặng', '🎯', 'milestone', 'weight_lost', 10, 500)
